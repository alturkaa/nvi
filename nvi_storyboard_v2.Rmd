...
df_final <- tracts_sf_index_orig_data %>% 
  mutate(AtRisk = ifelse(atrisk_gentrifying_dummy == 1, 'Yes', 'No'),
         Gentrifying = ifelse(gentrifying_using_educ_and_rent_or_hsg_value == 1, 'Yes', 'No'),
         rent_pct_increase = rent_pct_increase * 100,
         value_pct_increase = value_pct_increase * 100)
...
pal <- colorFactor(palette = "Blues", domain = tracts_sf_index_orig_data$Vulnerability)

area_popup <- paste0("<b>Census Tract:</b> ", tracts_sf_index_orig_data$Name,
                     "<br><b>Vulnerability:</b> ", tracts_sf_index_orig_data$Vulnerability,
                     "<br><b>Pct Rent Burdened:</b> ", tracts_sf_index_orig_data$percent_rent_burdened_2019,
                     "<br><b>Gentrifying?:</b> ", tracts_sf_index_orig_data$gentrifying_using_educ_and_rent_or_hsg_value,
                     "<br><b>Eviction Rate:</b> ", tracts_sf_index_orig_data$filing_rate_5yr,
                    '<br><b>Pct Uninsured:</b> ', tracts_sf_index_orig_data$percent_uninsured,
                    '<br><b>Poverty Rate:</b> ', tracts_sf_index_orig_data$poverty_rate_2019) %>%
...
Recent Rent and Eviction Data {data-navmenu="Overview"}
=========================================
...
Socioeconomic Factors {.storyboard data-navmenu="Dig Into the Data"}
=========================================
...
- A **`r rate_rent_burden_effect` percentage point increase** in the number of people in a Tract who are **rent burdened** is correlated with an **increase of 1 person (per 5,000)** falling into homelessness.
- On average, almost **`r rent_burden_q1_q3_effect_diff` more people (per 5,000)** fall into homelessness in the most rent burdened Tracts than they do in the least rent burdened Tracts.

*Median Gross Rent*

- An **`r rate_rent_increase_effect` percent increase** in the **median gross rent** in a Tract is correlated with an **increase of 1 person (per 5,000)** falling into homelessness.
- On average, almost **`r rent_increase_q1_q3_effect_diff` more people (per 5,000)** fall into homelessness in Tracts with the highest rent increase than they do in ones with the lowest.
...
- An increase of **`r rate_eviction_effect`** in the historical eviction filing rate in a Tract is correlated with an **increase of 1 person (per 5,000)** falling into homelessness.
- On average, almost **`r eviction_q1_q3_effect_diff` more people (per 5,000)** fall into homelessness in Tracts with the highest historical eviction rates than they do in Tracts with the least.

*Overcrowded Rental Units* 

- A **`r rate_overcrowded_effect` percentage point increase** in the number of people who live in **overcrowded rental units** is correlated with an **increase of 1 person (per 5,000)** falling into homelessness.
- On average, almost **`r overcrowded_q1_q3_effect_diff` more people (per 5,000)** fall into homelessness in Tracts with the highest number of overcrowded rental units than they do in Tracts with the least.
...
patch_hsg_plot <- hsg_values_homelessness_scatter + just_vulnerability_map + gentrifying_homelessness_scatter + plot_annotation(caption = "Hover over a Census tract to see its eviction filing rate and its homelessness rate.\nHovering over a dot in one of the plots allows you to see the location of that Census tract.\nClicking on the magnifying glass allows you to zoom into the map and plots.", theme = theme(plot.caption = element_text(size = 6)))
...
- On average, approximately **`r rate_gentrifying_effect` more people (per 5,000)** fall into homelessness in **gentrifying Tracts** than they do in Tracts that are not gentrifying.
...
patch_health_plots <- insurance_homelessness_scatter + just_vulnerability_map + health_services_homelessness_scatter + plot_annotation(caption = "Hover over a Census tract to see its health insurance coverage rate, the percent of people reporting bad mental health, and its homelessness rate.\nHovering over a dot in one of the scatterplots allows you to see the location of that Census tract.\nClicking on the magnifying glass allows you to zoom into the map and plots.", theme = theme(plot.caption = element_text(size = 6)))
...
health_services_q1_q3_effect_diff <- round((summary(df_final$allhealth_popden)[5] - summary(df_final$allhealth_popden)[2]) * health_services_model$coefficients[2], 0)
...
- A **`r rate_uninsured_effect` percentage point increase** in the number of people in a Tract who do not have **health insurance** is correlated with an **increase of 1 person (per 5,000)** falling into homelessness.

*Health Services*

- A decrease of **`r rate_health_services_effect*-1` health service facilities (per 1,000 people)** is correlated with an **increase of 1 person (per 5,000)** falling into homelessness.
...
patch_poverty_plots <- poverty_homelessness_scatter + just_vulnerability_map + unemployment_homelessness_scatter + plot_annotation(caption = "Hover over a Census tract to see its poverty and unemployment rates and its homelessness rate.\nHovering over a dot in one of the scatterplots allows you to see the location of that Census tract.\nClicking on the magnifying glass allows you to zoom into the map and plots.", theme = theme(plot.caption = element_text(size = 6)))
...
- A **`r rate_poverty_effect` percentage point increase** in the **poverty rate** in a Tract is correlated with an **increase of 1 person (per 5,000)** falling into homelessness.

*Unemployment*

- A **`r rate_unemployed_effect` percentage point increase** in the **unemployment rate** is correlated with an **increase of 1 person (per 5,000)** falling into homelessness.
...
- A **`r rate_black_effect` percentage point increase** in the proportion of a Tract's population that is **non-Hispanic Black** is correlated with an **increase of 1 person (per 5,000)** falling into homelessness.
- A **`r rate_hispanic_effect` percentage point increase** in the proportion of a Tract's population that is **Hispanic** is correlated with an **increase of 1 person (per 5,000)** falling into homelessness.
...
Table {data-navmenu="About the Data"}
=========================================
...
condensed_df <- df_final %>% 
  ...
  select(Name, total_pop, percent_rent_burdened_2019, percent_overcrowded_rentals, filing_rate_5yr, Gentrifying, percent_uninsured, allhealth_popden, poverty_rate_2019, Percent_Unemployed, percent_nhblack, percent_hispanic)
...
reactable(
  ...
  columnGroups = list(
    colGroup(name = "Rent Factors", columns = c("percent_rent_burdened_2019", "percent_overcrowded_rentals")),
...
About the Data and Index {data-navmenu="About the Data"}
=========================================
